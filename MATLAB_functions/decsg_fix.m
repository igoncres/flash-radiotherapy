...
if nargin > 1
    ...
    if isempty(sf)
       error(message('pdelib:decsg:NonEmptySF'));
    end
end
...
if nargin > 2
    ...
    if isempty(ns)
       error(message('pdelib:decsg:NonEmptyNS'));
    end
end
...
if(nargin == 2)
  error(message('pdelib:decsg:NarginError'));
end
...
while bf1
  ...
  if lsn>1
    ...
    % check for same line
    i=find(abs(dx1.*dy2-dx2.*dy1)<compTol & abs(dx1.*dy-dy1.*dx)<compTol);
    ...
    for c=ind
      ...
      y=[lsc(3,q),lsc(4,q)];
      [x,i]=sort(x);
      if abs(x(1)-x(2*length(q)))<compTol
        y=sort(y);
        p=0;
      else
        y=y(i);
      ...
      d=[true (abs(diff(x))>compTol | abs(diff(y))>compTol)];
      x=x(d);
      y=y(d);
      l=length(x);
      o1=ones(1,l-1);
      o2=ones(length(q),1);
      if p
        i1=lmm(2,q)'*o1-o2*x(2:l)>-compTol & lmm(1,q)'*o1-o2*x(1:l-1)<compTol;
      else
        i1=lmm(4,q)'*o1-o2*y(2:l)>-compTol & lmm(3,q)'*o1-o2*y(1:l-1)<compTol;
      end
      i2=logical(max(i1));
      ...
      if gdln>0
        ...
        l=1;
        for k=q'
          if (p && lsc(1,k)-lsc(2,k)<compTol) || (~p && lsc(3,k)-lsc(4,k)<compTol)
  ...
  if en>1       % Use bounding boxes as first intersection check
    ...
    bis=( ( (box1(1,:)<=box2(1,:)) & (box1(2,:)+compTol>=box2(1,:)-compTol) ) |...
          ( (box1(1,:)>=box2(1,:)) & (box1(1,:)-compTol<=box2(2,:)+compTol) ) ) &...
        ( ( (box1(3,:)<=box2(3,:)) & (box1(4,:)+compTol>=box2(3,:)-compTol) ) |...
          ( (box1(3,:)>=box2(3,:)) & (box1(3,:)-compTol<=box2(4,:)+compTol) ) );
    fbis=find(~bis);
    i1(fbis)=[]; i2(fbis)=[];
  end
  ...
  if en>1 && ~isempty(i1)        % Determine split-points
    ...
    for i=1:length(i1)
      ...
      j=find(abs(x(j1)-x(j2))<rootTol);
      if ~isempty(j)
        if length(j)==6
          x=0;
        elseif length(j)>2
          error(message('pdelib:decsg:RootMultGt2'));
        else
          if length(j)==2
          ...
          xp=roots(pxp);
          for jj=j(:)'
            jjj=find(abs(x(j1(jj))-xp)<rootTol);
            if length(jjj)~=1
              error(message('pdelib:decsg:InternalError'));
            end
            x(j1(jj))=xp(jjj);
            x(j2(jj))=2;
          end
      ...
      j=find(abs(x)<1+compTol&abs(imag(x))<compTol); 
      x=x(j);
      x=ea(i1(i))*x;
      ...
      if ~isempty(j)
        ...
        elseif length(j)>2
          ...
          yp=roots(pyp);
          for jj=j(:)'
            jjj=find(abs(y(j1(jj))-yp)<rootTol);
            if length(jjj)~=1
              error(message('pdelib:decsg:InternalError'));
            end
            y(j1(jj))=yp(jjj);
      ...
      j=find(abs(y)<1+compTol&abs(imag(y))<compTol); 
      y=y(j);
      y=eb(i1(i))*y;
      ...
      j=find(abs(A(i)*x.^2+B(i)*x.*y+C(i)*y.^2+D(i)*x+E(i)*y+F(i))<compTol);
      x=x(j); y=y(j);
  ...
  if en>0 && lsn>0 && ~isempty(i1)        % Determine split-points
    ...
    d=b.^2-4*a.*c;
    i=find(abs(d)<compTol);
    if ~isempty(i)
      j1=i1(i)'; j2=i2(i)';
      xi=lsc(1,j1)-b(i)./a(i)/2.*(lsc(2,j1)-lsc(1,j1));
      yi=lsc(3,j1)-b(i)./a(i)/2.*(lsc(4,j1)-lsc(3,j1));
      k=find((xi-lmm(1,j1)>compTol & xi-lmm(2,j1)<-compTol) | ...
             (yi-lmm(3,j1)>compTol & yi-lmm(4,j1)<-compTol));
  ...
  if cn>1 && ~isempty(i1)        % Determine split-points
    ...
    i=find(abs(d)<compTol);                       % same center
    d(i)=[]; i1(i)=[]; i2(i)=[];
    if ~isempty(d)
    b=(d.^2+cr(1,i1).^2-cr(1,i2).^2)./cr(1,i1)./d/2;
    i=find(b-(-1)>=compTol & b-1<=-compTol);
    if ~isempty(i)
  ...
  if lsn>1
    ...
    dt=dx1.*dy2-dx2.*dy1;
    i=find(abs(dt)>compTol);              % check for intersecting lines
    i1=i1(i);
    i2=i2(i);
    ...
    l1=find(((xi-lmm(1,i1)>compTol & xi-lmm(2,i1)<-compTol) | ...
             (yi-lmm(3,i1)>compTol & yi-lmm(4,i1)<-compTol)) & ...
            (xi-lmm(1,i2)>-compTol & xi-lmm(2,i2)<compTol & ...
             yi-lmm(3,i2)>-compTol & yi-lmm(4,i2)<compTol));
  ...
  if en>0
    ...
    for i=1:size(ec,2)
      ...
      if isempty(k)
        ...
        j=find(abs(t-pi)<angleTol);
        t(j)=-pi*ones(size(j));
        t=sort(t);
        d=[true abs(diff(t))>angleTol];
        t=t(d);
        t1=diff([t t(1)+2*pi]);
        j=find(t1-pi/2>angleTol&t1-pi<=angleTol);
        if ~isempty(j)
          t2=t(j)'+t1(j)'/2;
          t=[t t2(:)'];
        end
        j=find(t1-pi>angleTol&t1-3*pi/2<=angleTol);
        if ~isempty(j)
          t2=t(j)'*ones(1,2)+t1(j)'*[1 2]/3;
          t=[t t2(:)'];
        end
        j=find(t1-3*pi/2>angleTol);
        if ~isempty(j)
  ...
  if ~isempty(lpi)
    ...
    while i<=n
      ...
      y=y(d);
      l=length(x);
      if (p & lsc(1,lpi(i))-lsc(2,lpi(i))>compTol) | ...
         (~p & lsc(3,lpi(i))-lsc(4,lpi(i))>compTol)
        x=x(l:-1:1);
        y=y(l:-1:1);
      end
      i1=j+lsn;
  ...
  i=1; n=length(px); del=[];
  while i<n
    k=find(abs(px-px(i))<compTol);
    py(k)=sort(py(k));
    i=i+length(k);
  end

  del=find(abs(px(1:n-1)-px(2:n))<compTol & abs(py(1:n-1)-py(2:n))<compTol);
  px(del)=[];
  py(del)=[];
  ...
  if ~isempty(px1)
    ...
    while i<n
      k=find(abs(px1-px1(i))<compTol);
      py1(k)=sort(py1(k));
      i=i+length(k);
    end

    del=find(abs(px1(1:n-1)-px1(2:n))<compTol & abs(py1(1:n-1)-py1(2:n))<compTol);
    px1(del)=[];
    py1(del)=[];
  ...
  for i=1:length(px)
    x=px(i); y=py(i);
    if ~isempty(lsc)
      p1=find(abs(lsc(1,:)-x)<compTol & abs(lsc(3,:)-y)<compTol);
      a1=atan2(lsc(4,p1)-y,lsc(2,p1)-x);
      p2=find(abs(lsc(2,:)-x)<compTol & abs(lsc(4,:)-y)<compTol);
    ...
    c2=zeros(1,n2);
    if ~isempty(csc)
      p3=find(abs(csc(1,:)-x)<compTol & abs(csc(3,:)-y)<compTol);
      if ~isempty(p3)
        a3=cst(1,p3)+pi/2;
      ...
      p4=find(abs(csc(2,:)-x)<compTol & abs(csc(4,:)-y)<compTol);
      if ~isempty(p3)
        a4=cst(2,p4)-pi/2;
    ...
    n4=length(p4);
    if ~isempty(esc)
      p5=find(abs(esc(1,:)-x)<compTol & abs(esc(3,:)-y)<compTol);
      if ~isempty(p5)
      ...
      p6=find(abs(esc(2,:)-x)<compTol & abs(esc(4,:)-y)<compTol);
      if ~isempty(p5)
      ...
      for i=[k(1:length(k)-1);k(2:length(k))]
        ...
        for l1=full(find(em(i(2),:)))
          for l2=full(find(em(i(1),:)))
            if abs(sc(1,l1)-sc(1,l2))>compTol & abs(sc(3,l1)-sc(3,l2))>compTol
              x1=sc(1,l1);
              y1=sc(3,l1);
              x2=sc(1,l2);
              y2=sc(3,l2);
...
if isempty(si), dl1=[]; dl=[]; bt=[]; msb=[]; return, end
...
if ~bf2
  ...
  i=1; n=length(x); l=[];
  while i<=n
    k=find(abs(x-x(i))<compTol);
    yk=sort(y(k)); yn=length(yk); y(k)=yk;
    if yn==1
      l=[l,i];
    else
      ydiff=abs(yk(1:yn-1)-yk(2:yn))>compTol;
      if yn==2 & ydiff==1
        l=[l,i,i+1];
      elseif yn>2
  ...
  dm=size(dl,1);
  for i=1:n
    j=find(abs(dl1(2,:)-x(i))<compTol&abs(dl1(4,:)-y(i))<compTol);
    k=find(abs(dl1(3,:)-x(i))<compTol&abs(dl1(5,:)-y(i))<compTol);
    if length(j)>2
      error(message('pdelib:decsg:InternalError'));
    elseif length(j)==2
      if length(k)>0
